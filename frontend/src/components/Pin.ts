...
//           C12 38 32 64 32 64
//           C32 64 52 38 52 22
//           C52 10.954 43.046 2 32 2 Z
//         "
//       />
//       <circle cx="32" cy="22" r="8" fill="#fff" opacity="0.95" />
//     </svg>
//   );
// }
...
export function buildEmojiPinSvg(color: string, px: number): string {
  return `<?xml version="1.0" encoding="UTF-8"?>
    <svg xmlns="http://www.w3.org/2000/svg" width="${px}" height="${px}" viewBox="0 0 64 64">
      <rect x="31.4" y="26" width="1.2" height="38" rx="0.6" fill="#64748B" />
      <circle cx="32" cy="18" r="8" fill="${color}" stroke="rgba(255,255,255,0.5)" stroke-width="0.5" />
    </svg>`;
}

export async function addPinImageToMap(
  map: MLMap,
  id: string,
  color: string,
  size = 128,
  ...
  await rasterizeSvgToMapImage(map, id, url, size, pixelRatio);
}
...
function rasterizeSvgToMapImage(
  map: MLMap,
  id: string,
  url: string,
  size: number,
    ...
    try {
      ...
      img.onload = () => {
        try {
          ...
          (map as any).addImage(id, imageData, { pixelRatio });
          resolve();
        } catch (err) {
          reject(err);
        }
